;(function(){
  ...
  function applyThemeToLoader() {
    ...
    // Apply theme-specific styles to loader
    const isLight = savedTheme === 'light'
    loader.style.background = isLight ? '#ffffff' : '#0a0a0a'
    loader.style.color = isLight ? '#0a0a0a' : '#ffffff'
    
    // Update progress text color
    const progressText = document.getElementById('progressText')
    if (progressText) {
      progressText.style.color = isLight ? '#0a0a0a' : '#ffffff'
    }
    
    // Update progress bar background
    const progressBar = loader.querySelector('.progress-bar')
    if (progressBar) {
      progressBar.style.background = isLight ? '#e0e0e0' : '#333333'
    }
  }
  ...
  function initCacheManagement() {
    ...
    const cacheStatusBtn = document.getElementById('cacheStatusBtn')

    if (preCacheBtn) {
      preCacheBtn.addEventListener('click', async () => {
        if (window.cacheManager) {
          preCacheBtn.textContent = 'Кеширование...'
          preCacheBtn.disabled = true
          
          try {
            await window.cacheManager.preCachePages()
            showNotification('Страницы успешно кешированы!', 'success')
          } catch (error) {
            showNotification('Ошибка при кешировании страниц', 'error')
          } finally {
            preCacheBtn.textContent = 'Кешировать страницы'
    ...
    if (clearCacheBtn) {
      clearCacheBtn.addEventListener('click', async () => {
        if (window.cacheManager) {
          if (confirm('Вы уверены, что хотите очистить весь кеш?')) {
            ...
            try {
              ...
              clearCacheBtn.textContent = 'Очистить кеш'
    ...
    if (cacheStatusBtn) {
      cacheStatusBtn.addEventListener('click', async () => {
        if (window.cacheManager) {
          try {
            const status = await window.cacheManager.getCacheInfo()
            if (status) {
              showCacheStatusModal(status)
            } else {
              showNotification('Не удалось получить статус кеша', 'error')
            }
          } catch (error) {
            showNotification('Ошибка при получении статуса кеша', 'error')
          }
        }
      })
    }
  ...
  function showCacheStatusModal(status) {
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div style="margin-bottom: 20px;">
            <strong>Всего элементов в кеше:</strong> ${status.totalItems}
          </div>
          <div>
            <strong>Детали по кешам:</strong>
            <ul style="margin-top: 10px; padding-left: 20px;">
              ${Object.entries(status.caches).map(([name, info]) => 
                `<li><strong>${name}:</strong> ${info.size} элементов</li>`
              ).join('')}
            </ul>
          </div>
        </div>
  ...
  function showNotification(message, type = 'info') {
    ...
    notification.style.cssText = `
      ...
      top: 20px;
      right: 20px;
      background: ${type === 'success' ? 'var(--brand)' : type === 'error' ? '#ff6b6b' : 'var(--panel)'};
      color: white;
      padding: 12px 16px;
      border-radius: 8px;
      font-size: 14px;
  ...
  // Initialize cache management when DOM is ready
  if (document.readyState === 'loading') {
